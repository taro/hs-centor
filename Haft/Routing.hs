...
{-|
	Takes the contents of a single YObject which represents a 
	single route and parses it into a route.
-}
parseRoute :: YamlValue -> Route
parseRoute (YObject (YamlObject (_, attrs))) =
	-- Ugh, I should make a fromYaml function or something
	let unwrapHash (YHash x) = x in
	let unwrapPair (x, YString y) = (x, y) in

	-- Pull out some of the attributes
	let Just (YString url) = lookup "url" attrs in
	let params = map unwrapPair $ maybe [] unwrapHash $ lookup "params" attrs in

	-- then split the URL and re-fold it into an unbranching tree
	foldr f (RouteEnd $ Just params) $ parts url
	where
		parts url = url =~ "[^/]+" :: [String]
		f x a = case (x !! 0) of
...
-}
parseRoutes :: YamlObject -> [Route]
parseRoutes (YamlObject ("routing", routes)) = map (parseRoute . snd) routes
